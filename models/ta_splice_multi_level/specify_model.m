...
assert(length(exo_idx) == PAR.NUM_LEVELS);
ino_idx = strmatch('ino', fn);
assert(length(ino_idx) == PAR.NUM_LEVELS);
e_ss_idx = strmatch('e_ss', fn);
assert(length(e_ss_idx) == PAR.NUM_LEVELS);
i_ss_idx = strmatch('i_ss', fn);
assert(length(i_ss_idx) == PAR.NUM_LEVELS);
ei_ss_idx = strmatch('ei_ss', fn);
assert(length(ei_ss_idx) == PAR.NUM_LEVELS);
ie_ss_idx = strmatch('ie_ss', fn);
assert(length(ie_ss_idx) == PAR.NUM_LEVELS);
...
state_model(STATES.ige).trans_scores         = [1 ones(1,PAR.NUM_LEVELS)];
